...
actor class DRC721() {
    ...
    public shared query func allURIs() : async [Text] {
		return _allURIs();
	};

    private func _allURIs() : [Text] {
        var array : [Text] = [];
        for ((key, value) in owners.entries()) {
            let uri : ?Text = _tokenURI(key);
            switch(uri){
            case(null){
            };
            case(?something){
                array := Array.append(array, [something]);
            };
        };
    ...
    public shared query func allMinted() : async [Token] {
		return _allMinted();
	};
